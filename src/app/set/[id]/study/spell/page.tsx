...
export default function SpellPage() {
  ...
  function handleNext() {
    if (lastCorrect) engine.markCorrect()
    ...
    return (
      <div className="flex min-h-screen-safe flex-col">
        ...
        <main className="flex flex-1 flex-col items-center justify-center px-4 pb-24">
          ...
          <p className="mb-1 text-gray-600">{engine.correctCount} / {engine.total} correct</p>
      ...
      <main className="flex-1 px-4 pb-24 pt-4">
        ...
        {showResult ? (
          <div className="flex flex-col items-center gap-4">
            <div className={`w-full rounded-xl p-4 text-center ${lastCorrect ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>
              <p className="font-medium">{lastCorrect ? "Correct!" : "Incorrect"}</p>
              <p className="mt-1 text-sm">Correct answer: <strong>{engine.currentCard?.term}</strong></p>
            </div>
